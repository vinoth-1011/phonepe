...
if section == "Dashboard":
    ...
    with st.expander("Filters (optional)"):
        year_opt = st.selectbox("Year (for some cards/charts)", options=["All", 2021, 2022, 2023, 2024], index=0)
        q_opt = st.selectbox("Quarter (for some cards/charts)", options=["All", 1, 2, 3, 4], index=0)
    ...
    with center:
        ...
        if not warn_if_empty(map_df, "No map data."):
            map_df = map_df.copy()
            map_df["state_norm"] = map_df["state"].str.strip()
    ...
    with right:
        ...
        tabs = st.tabs(["Top 10 States", "Top 10 Districts"])
        with tabs[0]:
            q = """
                SELECT state, SUM(transaction_amount) AS total_amount
                FROM aggregated_transaction_data
            ...
            """
            d = run_sql(q)
            st.dataframe(d, use_container_width=True, height=330)
        with tabs[1]:
            q = """
                SELECT district_state_name, SUM(transaction_amount) AS total_amount
